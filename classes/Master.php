...
Class Master extends DBConnection {
	...
	function update_order_status(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `orders` SET `status` = '$status', `payment_status` = 1 WHERE id = '{$id}' ");
	
		if($update){
			$resp['status'] ='success';
			$this->settings->set_flashdata("success"," Order status and payment status successfully updated.");
		} else {
			$resp['status'] ='failed';
